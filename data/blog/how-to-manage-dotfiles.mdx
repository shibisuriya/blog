...
Dotfiles are hidden configuration files that are used to customize the behaviour
of various programs in Unix-based operating systems like GNU/Linux and macOS.
For example, the dotfile `.zshrc` (typically present in the user's home
directory - `/Users/<your-username>/`) can be used to customize the user's zsh
shell.
...
Let's say, recently at work your task involves making a lot of small changes to
the codebase and pushing them to the remote repeatedly (think testing a CI by
trail & error). After scambling through the clunky GUI of vscode to press the
"Push commit" button after commiting each change you feel that you will highly
benefit from adding a keyboard shortcut to push the commits to the remote. So
you add the keyboard shortcut and it feels very ergonomic, within hours/days
your mind/nervous system gets used to the keybinding & you start to take it for
granted. When you hop on your personal computer after work hours or in the
weekend you will badly missing this keybinding (trust me bro 😀)! You now have
to manually copy all the keybindings & settings that you changed in your work
computer to your personal computer typically by pressing buttons on a GUI (a
...
across your computers with very little effort especially for power users.
...
Typically, we tend to customize the computer programs that we use on a regular
basis to get our work done faster. Customizations might range from adding a few
custom key biddings or changing the programs appearance.

What if we were.

What if we were able to install all the programs we need,
...
- [Ansible playbook](https://docs.ansible.com/ansible/latest/playbook_guide/playbooks_intro.html) (optional, have replaced it with Nix package manager now)
...
My dotfiles repo is [public](https://github.com/shibisuriya/.dotfiles). If yours
is private, you must first set up your Git credentials.
...
Ones nix-darwin is installed, we will have access to the `darwin-rebuild`
command,
...
now I am satisfied with GNU Stow.

7. Update you git configuration (this step needs to be automated, I am
   performing this step manually for now, because I don't want to store my
   email, etc. in my public dotfiles repo).
